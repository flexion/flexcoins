...
func _ready() -> void:
	...
	for i in range(COIN_COUNT):
		...
		coin.position = Vector2(randf_range(40.0, 680.0), randf_range(-60.0, 1280.0))
		coin.rotation = randf_range(0.0, TAU)

		var speed: float = randf_range(MIN_SPEED, MAX_SPEED)
		var rot_speed: float = randf_range(-1.0, 1.0)

		# ~30% gold tint, rest warm modulate
		if randf() < 0.3:
			coin.modulate = Color(1.0, 0.9, 0.3, 0.8)
		else:
			coin.modulate = Color(1.0, 0.95, 0.85, 0.7)
...
func _process(delta: float) -> void:
	for i in range(coins.size()):
		coins[i].position.y += coin_speeds[i] * delta
		coins[i].rotation += coin_rotation_speeds[i] * delta
		if coins[i].position.y > 1340.0:
			coins[i].position.y = -60.0
			coins[i].position.x = randf_range(40.0, 680.0)
